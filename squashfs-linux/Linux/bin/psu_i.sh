...
function SetPSUParameters
{
	...
	if [ "$DellPartNumber" = "TW16K" ]; then
	...
	elif [ "$DellPartNumber" = "N24MJ" ] || [ "$DellPartNumber" = "3GHW3" ] || [ "$DellPartNumber" = "13MD5" ]; then
		...
		Efficiency_hl_50="94"
		Efficiency_hl_100="91"
	elif [ "$DellPartNumber" = "5NF18" ] || [ "$DellPartNumber" = "6W2PW" ] || [ "$DellPartNumber" = "79RDR" ] || [ "$DellPartNumber" = "9PXCV" ] || [ "$DellPartNumber" = "D5MW8" ] || [ "$DellPartNumber" = "F9F51" ] || [ "$DellPartNumber" = "FT7H2" ] || [ "$DellPartNumber" = "H40WN" ] || [ "$DellPartNumber" = "KRT26" ] || [ "$DellPartNumber" = "N30P9" ] || [ "$DellPartNumber" = "RWDG4" ] || [ "$DellPartNumber" = "W0CTF" ]; then
	...
	elif [ "$DellPartNumber" = "38GYJ" ] || [ "$DellPartNumber" = "CC6WF" ] || [ "$DellPartNumber" = "DX4F8" ] || [ "$DellPartNumber" = "GDPF3" ] || [ "$DellPartNumber" = "GYH9V" ] || [ "$DellPartNumber" = "HFRHN" ] || [ "$DellPartNumber" = "HT6GX" ] || [ "$DellPartNumber" = "NTCWP" ] || [ "$DellPartNumber" = "YT39Y" ]; then
	...
	elif [ "$DellPartNumber" = "02RN7" ] || [ "$DellPartNumber" = "C7JTF" ] || [ "$DellPartNumber" = "K6XVM" ] || [ "$DellPartNumber" = "Y1MGX" ]; then
		...
		Efficiency_ll_10=""
		Efficiency_ll_20="91"
		Efficiency_ll_50="92.5"
		Efficiency_ll_100="90"
		Efficiency_hl_Voltage="230"
		Efficiency_hl_10=""
		Efficiency_hl_20="93"
		Efficiency_hl_50="94.5"
		...
		echo
		echo "Unrecognised Dell PSU part number from FRU data."
		echo
		exit
	fi
}
...
function send_Cmd()
{
	...
	if [[ $# -lt 2 ]]; then
		echo "Call made to function 'send_Cmd' with insufficient number of parameters ($#)."
		exit
	fi
...
#		echo "buf = $buf"
		Found=`expr match "$Resp" "$buf"`

#		echo "Found = $Found"

		if [ "$Found" = "$__Response" ]; then
			(( RC = 0 ))
		else
			(( RC = 1 ))
		fi
...
function ParseMiddle()
{
	...
	if [[ $# -lt 4 ]]; then
		echo "Call made to function 'ParseMiddle' with insufficient number of parameters ($#)."
		exit
	fi
...
function ParseLeft()
{
	...
	if [[ $# -lt 3 ]]; then
		echo "Call made to function 'ParseLeft' with insufficient number of parameters ($#)."
		exit
	fi
...
function ParseRight()
{
	...
	if [[ $# -lt 3 ]]; then
		echo "Call made to function 'ParseRight' with insufficient number of parameters ($#)."
		exit
	fi
...
function page_plus_write
{
	...
	if [ "$STSCmd" = "byte" ]; then
		...
		if [ "$Generation" = "12G" ]; then
			echo "page_plus_write - 'ocw_status' not supported by 12G PSUs."
			return 0
		fi
	else
...
function page_plus_read
{
	...
	if [ "$STSCmd" = "byte" ]; then
		...
		if [ "$Generation" = "12G" ]; then
			echo "page_plus_read - 'ocw_status' not supported by 12G PSUs."
			return 0
		fi
		STS="0xdd"
	else
...
function firmware_update_capability
{
	...
	if [ "$Generation" = "12G" ] ; then
		echo "Command 'firmware_update_capability' is not supported on 12G PSUs."
		return 0
	fi
...
function firmware_update_status_flag
{
	...
	if [ "$Generation" = "12G" ] ; then
		echo "Command 'firmware_update_status_flag' is not supported on 12G PSUs."
		return 0
	fi
...
function psu_manufacturing
{
	...
	if [ "$Generation" = "12G" ]; then
		echo "12G PSUs do not support command 'psu_manufacturing'."
		return 0
	fi
...
function iout_oc_fault_limit
{
	...
	if [ "$WR" = "1" ]; then
		...
		if [ "$Generation" = "12G" ] ; then
			echo "12G PSUs do not support 'iout_oc_fault_limit' writes."
			return 0
		fi
...
#	echo "Manufacturing Date And Time = $Minutes minutes since 0:00 hrs on 1/1/96."
	...
	if [ "$Generation" = "13G" ]; then
		...
		if [ "$Generation" = "12G" ]; then
			if [ "$FRUFileIDData" != "04" ]; then
				echo
				echo " *** Unexpected value at offset '4B'h - FRU File ID Data ***"
				echo "Expected '04'h, read '$FRUFileIDData'h."
				echo "++++++++++ Compare Error ++++++++++"
				echo
				(( RC = 1 ))
			else
				echo "Offset '4B'h: FRU File ID Data = '04'h - ('04'h for 12G platform)."
			fi
		fi
	fi
...
function mfr_efficiency_data
{
	...
	if [ "$Generation" = "12G" ] ; then
		echo "Command 'mfr_efficiency_data' is not supported on 12G PSUs."
		return 0
	fi
...
function bootloader_version
{
	...
	if [ "$Generation" = "12G" ] ; then
		echo "Command 'bootloader_version' is not supported on 12G PSUs."
		return 0
	fi
...
function ocw_setting_write
{
	...
	if [ "$Generation" = "12G" ] ; then
		echo "Command 'ocw_setting_read' is not supported on 12G PSUs."
		return 0
	fi
...
function ocw_setting_range
{
	...
	if [ "$Generation" = "12G" ] ; then
		echo "Command 'ocw_setting_range' is not supported on 12G PSUs."
		return 0
	fi
	...
	if [ "$WR" = "1" ]; then
		...
		if [ "$InpBV" = "fixed" ]; then
			if [ "$Generation" = "13G" ]; then
				...
				echo "Input Bulk Voltage setting not supported by 12G PSUs"
				return 0
			fi
...
function Power()
{
	...
	local Save=""
	local ExpectedValue="ExpectedValue"
	local ActualValue="Initialized"

	local PSU1EnergyCountIn="EnergyCountIn"
	local PSU1RolloverCountIn="RolloverCountIn"
	local -i PSU1RolloverRolloverCountIn=0
	local -i PSU1RolloverRolloverCountInFlag="0"
	local PSU1SampleCountIn="SampleCountIn"
	local PSU1EnergyCountInLast="EnergyCountInLast"
	local PSU1RolloverCountInLast="RolloverCountInLast"
	local PSU1SampleCountInLast="SampleCountInLast"
	local PSU1TotalEnergyCountOutLast="PSU1TotalEnergyCountOutLast"
	local PSU1TotalEnergyCountOut="PSU1TotalEnergyCountOut"
	local PSU1EnergyCountOut="EnergyCountOut"
	local PSU1RolloverCountOut="RolloverCountOut"
	local -i PSU1RolloverRolloverCountOut=0
	local -i PSU1RolloverRolloverCountOutFlag="0"
	local PSU1SampleCountOut="SampleCountOut"
	local PSU1EnergyCountOutLast="EnergyCountOutLast"
	local PSU1RolloverCountOutLast="RolloverCountOutLast"
	local PSU1SampleCountOutLast="SampleCountOutLast"

	local PSU2EnergyCountIn="EnergyCountIn"
	local PSU2RolloverCountIn="RolloverCountIn"
	local -i PSU2RolloverRolloverCountIn=0
	local -i PSU2RolloverRolloverCountInFlag="0"
	local PSU2SampleCountIn="SampleCountIn"
	local PSU2EnergyCountInLast="EnergyCountInLast"
	local PSU2RolloverCountInLast="RolloverCountInLast"
	local PSU2SampleCountInLast="SampleCountInLast"
	local PSU2TotalEnergyCountOutLast="PSU2TotalEnergyCountOutLast"
	local PSU2TotalEnergyCountOut="PSU2TotalEnergyCountOut"
	local PSU2EnergyCountOut="EnergyCountOut"
	local PSU2RolloverCountOut="RolloverCountOut"
	local -i PSU2RolloverRolloverCountOutFlag="0"
	local -i PSU2RolloverRolloverCountOut=0
	local PSU2EnergyCountOutLast="EnergyCountOutLast"
	local PSU2RolloverCountOutLast="RolloverCountOutLast"
	local PSU2SampleCountOutLast="SampleCountOutLast"
	local -i PSU1SampleRolloverCountOut=0
	local PSU1SampleRolloverCountOutFlag="1"
	local -i PSU1SampleRolloverCountIn=0
	local PSU1SampleRolloverCountInFlag="1"
	local -i PSU2SampleRolloverCountOut=0
	local PSU2SampleRolloverCountOutFlag="1"
	local -i PSU2SampleRolloverCountIn=0
	local PSU2SampleRolloverCountInFlag="1"
...
echo "***** Beginning validation *****"
	...
	if [ "$OtherPSU" != "0" ]; then
		coefficients $OtherPSU "read_ein" ActualValue
		if [[ $? -ne 0 ]]; then
			(( Failed++ ))
			echo "     ++++++++++ Transaction Error ++++++++++"
		else
	...
	if [ "$OtherPSU" != "0" ]; then
		coefficients $OtherPSU "read_eout" ActualValue
		if [[ $? -ne 0 ]]; then
			(( Failed++ ))
			echo "     ++++++++++ Transaction Error ++++++++++"
		else
	...
	read_ein $PSU PSU1EnergyCountInLast PSU1RolloverCountInLast PSU1SampleCountInLast
	if [[ $? -ne 0 ]]; then
		(( Failed++ ))
		echo "     ++++++++++ Transaction Error ++++++++++"
	else
		(( PSU1TotalEnergyCountInLast = $PSU1RolloverCountInLast * $Ymax + $PSU1EnergyCountInLast ))
		echo "PSU1 Energy In Count = $PSU1TotalEnergyCountInLast"
		echo
	fi

	if [ "$OtherPSU" != "0" ]; then
		read_ein $OtherPSU PSU2EnergyCountInLast PSU2RolloverCountInLast PSU2SampleCountInLast
		if [[ $? -ne 0 ]]; then
			(( Failed++ ))
			echo "     ++++++++++ Transaction Error ++++++++++"
		else
			(( PSU2TotalEnergyCountInLast = $PSU2RolloverCountInLast * $Ymax + $PSU2EnergyCountInLast ))
			echo "PSU2 Energy In Count = $PSU2 TotalEnergyCountInLast"
			echo
		fi
	fi

	read_eout $PSU PSU1EnergyCountOutLast PSU1RolloverCountOutLast PSU1SampleCountOutLast
	if [[ $? -ne 0 ]]; then
		(( Failed++ ))
		echo "     ++++++++++ Transaction Error ++++++++++"
	else
		(( PSU1TotalEnergyCountOutLast = $PSU1RolloverCountOutLast * $Ymax + $PSU1EnergyCountOutLast ))
		echo "PSU1 Energy Out Count = $PSU1TotalEnergyCountOutLast"
	fi

	if [ "$OtherPSU" != "0" ]; then
		read_eout $OtherPSU PSU2EnergyCountOutLast PSU2RolloverCountOutLast PSU2SampleCountOutLast
		if [[ $? -ne 0 ]]; then
			(( Failed++ ))
			echo "     ++++++++++ Transaction Error ++++++++++"
		else
			(( PSU2TotalEnergyCountOutLast = $PSU2RolloverCountOutLast * $Ymax + $PSU2EnergyCountOutLast ))
			echo "PSU2 Energy Out Count = $PSU2TotalEnergyCountOutLast"
		fi
	fi

	send_Cmd "racadm set system.power.EnergyConsumption.Clear 1" "Object value modified successfully"
	if [[ $? -ne 0 ]]; then
		(( Failed++ ))
		echo "     ++++++++++ Transaction Error ++++++++++"
		exit
	...
	if [ "$OtherPSU" != "0" ]; then
		mfr_id $OtherPSU DellPartNumber
		if [[ $? -ne 0 ]]; then
			(( Failed++ ))
			echo "     ++++++++++ Transaction Error ++++++++++"
		fi
	...
	while [ "$Stop" != "1" ] ; do
		...
		PSU2AveragePowerIn=0;

		read_ein $PSU PSU1EnergyCountIn PSU1RolloverCountIn PSU1SampleCountIn
		if [[ $? -ne 0 ]]; then
			(( Failed++ ))
			echo "     ++++++++++ Transaction Error ++++++++++"
		else

			if [[ $PSU1RolloverCountIn -lt $PSU1RolloverCountInLast ]] && [ $PSU1RolloverRolloverCountInFlag = "0" ]; then
				(( PSU1RolloverRolloverCountIn++ ))
				PSU1RolloverRolloverCountInFlag="1"
			elif [[ $PSU1RolloverCountIn -ge $PSU1RolloverCountInLast ]] && [ $PSU1RolloverRolloverCountInFlag = "1" ]; then
				PSU1RolloverRolloverCountInFlag="0"
			fi

			(( PSU1TotalEnergyCountIn = ($PSU1RolloverCountIn + 256 * $PSU1RolloverRolloverCountIn) * $Ymax + $PSU1EnergyCountIn ))
			echo "PSU1 Energy In Count = $PSU1TotalEnergyCountIn"
			echo
		fi

		read_eout $PSU PSU1EnergyCountOut PSU1RolloverCountOut PSU1SampleCountOut
		if [[ $? -ne 0 ]]; then
			(( Failed++ ))
			echo "     ++++++++++ Transaction Error ++++++++++"
		else
			if [[ $PSU1RolloverCountOut -lt $PSU1RolloverCountOutLast ]] && [ $PSU1RolloverRolloverCountOutFlag = "0" ]; then
				(( PSU1RolloverRolloverCountOut++ ))
				PSU1RolloverRolloverCountOutFlag="1"
			elif [[ $PSU1RolloverCountOut -ge $PSU1RolloverCountOutLast ]] && [ $PSU1RolloverRolloverCountOutFlag = "1" ]; then
				PSU1RolloverRolloverCountOutFlag="0"
			fi

			(( PSU1TotalEnergyCountOut = ($PSU1RolloverCountOut + 256 * $PSU1RolloverRolloverCountOut) * $Ymax + $PSU1EnergyCountOut ))
			echo "PSU1 Energy Out Count = $PSU1TotalEnergyCountOut"
			echo
		fi

		if [ "$OtherPSU" != "0" ]; then
			read_ein $OtherPSU PSU2EnergyCountIn PSU2RolloverCountIn PSU2SampleCountIn
			if [[ $? -ne 0 ]]; then
				(( Failed++ ))
				echo "     ++++++++++ Transaction Error ++++++++++"
			else
				if [[ $PSU2RolloverCountIn -lt $PSU2RolloverCountInLast ]] && [ $PSU2RolloverRolloverCountInFlag = "0" ]; then
					(( PSU2RolloverRolloverCountIn++ ))
					PSU2RolloverRolloverCountInFlag="1"
				elif [[ $PSU2RolloverCountIn -ge $PSU2RolloverCountInLast ]] && [ $PSU2RolloverRolloverCountInFlag = "1" ]; then
					PSU2RolloverRolloverCountInFlag="0"
				fi

				(( PSU2TotalEnergyCountIn = ($PSU2RolloverCountIn + 256 * $PSU2RolloverRolloverCountIn) * $Ymax + $PSU2EnergyCountIn ))
				echo "PSU2 Energy In Count = $PSU2TotalEnergyCountIn"
				echo
			fi

			read_eout $OtherPSU PSU2EnergyCountOut PSU2RolloverCountOut PSU2SampleCountOut
			if [[ $? -ne 0 ]]; then
				(( Failed++ ))
				echo "     ++++++++++ Transaction Error ++++++++++"
			else
				if [[ $PSU2RolloverCountOut -lt $PSU2RolloverCountOutLast ]] && [ $PSU2RolloverRolloverCountOutFlag = "0" ]; then
					(( PSU2RolloverRolloverCountOut++ ))
					PSU2RolloverRolloverCountOutFlag="1"
				elif [[ $PSU2RolloverCountOut -ge $PSU2RolloverCountOutLast ]] && [ $PSU2RolloverRolloverCountOutFlag = "1" ]; then
					PSU2RolloverRolloverCountOutFlag="0"
				fi

				(( PSU2TotalEnergyCountOut = ($PSU2RolloverCountOut + 256 * $PSU2RolloverRolloverCountOut) * $Ymax + $PSU2EnergyCountOut ))
				echo "PSU2 Energy Out Count = $PSU2TotalEnergyCountOut"
				echo
			fi
		fi
		...
		if [[ $PSU1SampleCountIn -lt $PSU1SampleCountInLast ]] && [ $PSU1SampleRolloverCountOutFlag = "0" ]; then
			(( PSU1SampleRolloverCountOut++ ))
			PSU1SampleRolloverCountOutFlag="1"
		elif [[ $PSU1SampleCountIn -ge $PSU1SampleCountInLast ]] && [ $PSU1SampleRolloverCountOutFlag = "1" ]; then
			PSU1SampleRolloverCountOutFlag="0"
		fi

		(( PSU1AveragePowerIn = (($PSU1TotalEnergyCountIn - $PSU1TotalEnergyCountInLast) * 1000) / (($PSU1SampleCountIn + $PSU1SampleRolloverCountIn * 16777216) - $PSU1SampleCountInLast) ))

		ActualValue="$(( ($PSU1AveragePowerIn % 1000) + 5 ))"
		while [[ ${#ActualValue} -lt 3 ]]; do
		...
		echo
		echo "PSU1 Average Power In during test script     = $(( PSU1AveragePowerIn / 1000 )).${ActualValue:0:2} W"

		if [[ $PSU1SampleCountOut -lt $PSU1SampleCountOutLast ]] && [ $PSU1SampleRolloverCountOutFlag = "0" ]; then
			(( PSU1SampleRolloverCountOut++ ))
			PSU1SampleRolloverCountOutFlag="1"
		elif [[ $PSU1SampleCountOut -ge $PSU1SampleCountOutLast ]] && [ $PSU1SampleRolloverCountOutFlag = "1" ]; then
			PSU1SampleRolloverCountOutFlag="0"
		fi

		(( PSU1AveragePowerOut = (($PSU1TotalEnergyCountOut - $PSU1TotalEnergyCountOutLast) * 1000) / (($PSU1SampleCountOut + $PSU1SampleRolloverCountOut * 16777216) - $PSU1SampleCountOutLast) ))

		ActualValue="$(( ($PSU1AveragePowerOut % 1000) + 5))"
		while [[ ${#ActualValue} -lt 3 ]]; do
		...
		echo
		echo "PSU1 Average Power Out during test script    = $(( $PSU1AveragePowerOut / 1000 )).${ActualValue:0:2} W"
		echo

		(( Effic = ($PSU1AveragePowerOut * 10000) / $PSU1AveragePowerIn ))
		ActualValue="$(( ($Effic % 100) + 5))"
		while [[ ${#ActualValue} -lt 2 ]]; do
		...
		DateCmd="date +%s"
		(( ScriptTime = $($DateCmd | tail -n 1 ) - $ScriptStartTime ))
		echo "PSU1 Efficiency during test script execution = $(( $Effic / 100 )).${ActualValue:0:1}%"

		PSU1DecimalEnergy="$(( (($PSU1AveragePowerIn * $ScriptTime / 60 / 60 / 100 + 5) / 10) % 1000 ))"
		while [[ ${#PSU1DecimalEnergy} -lt 3 ]]; do

			PSU1DecimalEnergy="0$PSU1DecimalEnergy"
		done
		echo "PSU1 Accumulated energy consumption during test script execution = $(( $PSU1AveragePowerIn * $ScriptTime / 60 / 60 / 1000 / 1000 )).$PSU1DecimalEnergy kWh"

		if [ "$OtherPSU" != "0" ]; then
			if [[ $PSU2SampleCountIn -lt $PSU2SampleCountInLast ]] && [ $PSU2SampleRolloverCountInFlag = "0" ]; then
				(( PSU2SampleRolloverCountIn++ ))
				PSU2SampleRolloverCountInFlag="1"
			elif [[ $PSU2SampleCountIn -ge $PSU2SampleCountInLast ]] && [ $PSU2SampleRolloverCountInFlag = "1" ]; then
				PSU2SampleRolloverCountInFlag="0"
			fi

			(( PSU2AveragePowerIn = (($PSU2TotalEnergyCountIn - $PSU2TotalEnergyCountInLast) * 1000) / (($PSU2SampleCountIn + $PSU2SampleRolloverCountIn * 16777216) - $PSU2SampleCountInLast) ))

			ActualValue="$(( ($PSU2AveragePowerIn % 1000) + 5 ))"
			while [[ ${#ActualValue} -lt 3 ]]; do
			...
			echo
			echo "PSU2 Average Power In during test script     = $(( PSU2AveragePowerIn / 1000 )).${ActualValue:0:2} W"

			if [[ $PSU2SampleCountOut -lt $PSU2SampleCountOutLast ]] && [ $PSU2SampleRolloverCountOutFlag = "0" ]; then
				(( PSU2SampleRolloverCountOut++ ))
				PSU2SampleRolloverCountOutFlag="1"
			elif [[ $PSU2SampleCountOut -ge $PSU2SampleCountOutLast ]] && [ $PSU2SampleRolloverCountOutFlag = "1" ]; then
				PSU2SampleRolloverCountOutFlag="0"
			fi

			(( PSU2AveragePowerOut = (($PSU2TotalEnergyCountOut - $PSU2TotalEnergyCountOutLast) * 1000) / (($PSU2SampleCountOut + $PSU2SampleRolloverCountOut * 16777216) - $PSU2SampleCountOutLast) ))

			ActualValue="$(( ($PSU2AveragePowerOut % 1000) + 5))"
			while [[ ${#ActualValue} -lt 3 ]]; do
			...
			echo
			echo "PSU2 Average Power Out during test script    = $(( $PSU2AveragePowerOut / 1000 )).${ActualValue:0:2} W"
			echo

			(( Effic = ($PSU2AveragePowerOut * 10000) / $PSU2AveragePowerIn ))
			ActualValue="$(( ($Effic % 100) + 5))"
			while [[ ${#ActualValue} -lt 2 ]]; do
			...
			DateCmd="date +%s"
			(( ScriptTime = $($DateCmd | tail -n 1 ) - $ScriptStartTime ))
			echo "PSU2 Efficiency during test script execution = $(( $Effic / 100 )).${ActualValue:0:1}%"

			PSU2DecimalEnergy="$(( (($PSU2AveragePowerIn * $ScriptTime / 60 / 60 / 100 + 5) / 10) % 1000 ))"
			while [[ ${#PSU2DecimalEnergy} -lt 3 ]]; do

				PSU2DecimalEnergy="0$PSU2DecimalEnergy"
			done
			echo "PSU2 Accumulated energy consumption during test script execution = $(( $PSU2AveragePowerIn * $ScriptTime / 60 / 60 / 1000 / 1000 )).$PSU2DecimalEnergy kWh"

			TotalDecimalEnergy="$(( ((($PSU1AveragePowerIn + $PSU2AveragePowerIn) * $ScriptTime / 60 / 60 / 100 + 5) / 10) % 1000 ))"
			while [[ ${#TotalDecimalEnergy} -lt 3 ]]; do

				TotalDecimalEnergy="0$TotalDecimalEnergy"
			done
		...
		send_Cmd "racadm get system.power.EnergyConsumption" "KWh | " CmdResponse
		if [[ $? -ne 0 ]]; then
			(( Failed++ ))
			echo "     ++++++++++ Transaction Error ++++++++++"
			exit
...
#		echo $CmdResponse
		...
		ParseLeft "$CmdResponse" " KWh | " ActualValue

		echo
		echo "iDRAC reports system accumulated energy consumption = $ActualValue kWh"
		echo "   Calculated system accumulated energy consumption = $(( ($PSU1AveragePowerIn + $PSU2AveragePowerIn) * $ScriptTime / 60 / 60 / 1000 / 1000 )).$TotalDecimalEnergy kWh"
		echo

		DateCmd="date +%s"
		(( ScriptTime = $($DateCmd | tail -n 1 ) - $ScriptStartTime ))
		echo "Script has been executing for $(( $ScriptTime / 60 )) minutes and $(( $ScriptTime % 60 )) seconds."

		sleep 5

		if [ "$Duration" != "0" ]; then
			(( iDuration = $Duration ))
			if [[ ScriptTime -gt $(( $Duration  * 60)) ]]; then
				Stop="1"
			fi
		fi
